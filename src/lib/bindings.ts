...
async invalidateVolumeStatisticsCache() : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("invalidate_volume_statistics_cache") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async renameAssetTag(source: string, target: string, merge: boolean) : Promise<Result<RenameTagResult, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("rename_asset_tag", { source, target, merge }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type AssetSummary = { id: string; assetType: AssetType; name: string; creator: string; category: string | null; imageFilename: string | null; hasMemo: boolean; dependencies: string[]; boothItemId: number | null; publishedAt: number | null; tags: string[]; supportedAvatars: string[] }
...
export type BoothAssetInfo = { id: number; name: string; creator: string; category: string | null; imageUrls: string[]; publishedAt: number; estimatedAssetType: AssetType | null }
...
export type SortBy = "Name" | "Creator" | "CreatedAt" | "PublishedAt" | "BoothId"
